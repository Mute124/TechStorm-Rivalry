...
namespace TechStorm {
	...
	public:
		...
		//---- Text
		string text = "Button";
		uVec2f textSize = MeasureTextEx(font, text.c_str(), fontSize, fontSpacing),
		...
		*/
		...
		int updateObjects()
		{
			...
			// Mouse position
			uVec2f mouse_pos = GetMousePosition();
		...
		bool IsClicked()
		{
			...
			uVec2f Mouse = GetMousePosition();
			this->position.x = this->rect.x;
			this->position.y = this->rect.y;

			uVec2f Boundries = { position.x + rect.width,
