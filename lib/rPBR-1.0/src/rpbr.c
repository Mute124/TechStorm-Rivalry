...
int main()
{
    ...
    environment = pbr_LoadEnvironment(PATH_TEXTURES_HDR, CUBEMAP_SIZE, IRRADIANCE_SIZE, PREFILTERED_SIZE, BRDF_SIZE);

    // Load external resources
    model = LoadModel(PATH_MODEL);
    matPBR = pbr_SetupMaterialPBR(environment, (Color){ 255, 255, 255, 255 }, 255, 255);
#if defined(PATH_TEXTURES_ALBEDO)
    pbr_SetMaterialTexturePBR(&matPBR, PBR_ALBEDO, LoadTexture(PATH_TEXTURES_ALBEDO));
    ...
    pbr_SetMaterialTexturePBR(&matPBR, PBR_NORMALS, LoadTexture(PATH_TEXTURES_NORMALS));
    ...
    pbr_SetMaterialTexturePBR(&matPBR, PBR_METALNESS, LoadTexture(PATH_TEXTURES_METALNESS));
    ...
    pbr_SetMaterialTexturePBR(&matPBR, PBR_ROUGHNESS, LoadTexture(PATH_TEXTURES_ROUGHNESS));
    ...
    pbr_SetMaterialTexturePBR(&matPBR, PBR_AO, LoadTexture(PATH_TEXTURES_AO));
    SetTextureFilter(matPBR.ao.bitmap, FILTER_BILINEAR);
    textures[PBR_AO] = matPBR.ao.bitmap;
#endif
#if defined(PATH_TEXTURES_EMISSION)
    pbr_SetMaterialTexturePBR(&matPBR, PBR_EMISSION, LoadTexture(PATH_TEXTURES_EMISSION));
    ...
    pbr_SetMaterialTexturePBR(&matPBR, PBR_HEIGHT, LoadTexture(PATH_TEXTURES_HEIGHT));
    ...
    Light lights[MAX_LIGHTS] = {
        pbr_CreateLight(LIGHT_POINT, (Vector3){ LIGHT_DISTANCE, LIGHT_HEIGHT, 0.0f }, (Vector3){ 0.0f, 0.0f, 0.0f }, (Color){ 255, 0, 0, 255 }, environment),
        pbr_CreateLight(LIGHT_POINT, (Vector3){ 0.0f, LIGHT_HEIGHT, LIGHT_DISTANCE }, (Vector3){ 0.0f, 0.0f, 0.0f }, (Color){ 0, 255, 0, 255 }, environment),
        pbr_CreateLight(LIGHT_POINT, (Vector3){ -LIGHT_DISTANCE, LIGHT_HEIGHT, 0.0f }, (Vector3){ 0.0f, 0.0f, 0.0f }, (Color){ 0, 0, 255, 255 }, environment),
        pbr_CreateLight(LIGHT_DIRECTIONAL, (Vector3){ 0.0f, LIGHT_HEIGHT*2.0f, -LIGHT_DISTANCE }, (Vector3){ 0.0f, 0.0f, 0.0f }, (Color){ 255, 0, 255, 255 }, environment)
    };
    int totalLights = pbr_GetLightsCount();
    ...
    while (!WindowShouldClose())
    {
        ...
        if (resetScene || IsKeyPressed(KEY_R))
        {
            ...
            for (int i = 0; i < totalLights; i++)
            {
                ...
                pbr_UpdateLightValues(environment, lights[i]);
            }
        ...
        if (IsFileDropped())
        {
            ...
            {
                pbr_UnloadEnvironment(environment);
                environment = pbr_LoadEnvironment(droppedFiles[0], CUBEMAP_SIZE, IRRADIANCE_SIZE, PREFILTERED_SIZE, BRDF_SIZE);
                ...
                if (textures[PBR_ALBEDO].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_ALBEDO, textures[PBR_ALBEDO]);
                if (textures[PBR_NORMALS].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_NORMALS, textures[PBR_NORMALS]);
                if (textures[PBR_METALNESS].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_METALNESS, textures[PBR_METALNESS]);
                if (textures[PBR_ROUGHNESS].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_ROUGHNESS, textures[PBR_ROUGHNESS]);
                if (textures[PBR_AO].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_AO, textures[PBR_AO]);
                if (textures[PBR_EMISSION].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_EMISSION, textures[PBR_EMISSION]);
                if (textures[PBR_HEIGHT].id != 0) pbr_SetMaterialTexturePBR(&matPBR, PBR_HEIGHT, textures[PBR_HEIGHT]);
                ...
                if (supportedImage)
                {
                    for (int i = 0; i < MAX_TEXTURES; i++)
                    {
                        ...
                        if (CheckCollisionPointRec(GetMousePosition(), rect))
                        {
                            ...
                            if (textures[i].id != 0) pbr_UnsetMaterialTexturePBR(&matPBR, i);
                            pbr_SetMaterialTexturePBR(&matPBR, i, newTex);
                            textures[i] = newTex;
                            break;
                        }
                    }
        ...
        for (int i = 0; i < MAX_TEXTURES; i++)
        {
            ...
            {
                pbr_UnsetMaterialTexturePBR(&matPBR, i);
        ...
        pbr_UpdateEnvironmentValues(environment, camera, screenRes);
        ...
        BeginDrawing();
            ...
            BeginTextureMode(fxTarget);
                ...
                Begin3dMode(camera);
                    ...
                    pbr_DrawModelPBR(model, matPBR, (Vector3){ 0.0f, 0.0f, 0.0f }, (Vector3){ 0.0f, 1.0f, 0.0f }, 0.0f, (Vector3){ MODEL_SCALE, MODEL_SCALE, MODEL_SCALE });
                    ...
                    if (drawSkybox) pbr_DrawSkybox(environment, camera);
...
void DrawLightInterface(Light *light)
{
    ...
    // Send lights values to environment PBR shader
    pbr_UpdateLightValues(environment, *light);
}
