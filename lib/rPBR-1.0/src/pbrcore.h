 ...
 *       Remember to call pbr_UnloadMaterialPBR and pbr_UnloadEnvironment to deallocate required memory and unload textures
...
{ // Prevents name mangling of functions
    ...
    static int pbr_lightsCount = 0;

    void pbr_RenderCube(void);
    void pbr_RenderQuad(void);

    void pbr_UpdateLightValues(pbr_env env, Light light);
    //------------ ----------------------------------------------------------------------
    // Functions Declaration
    //----------------------------------------------------------------------------------
    // Set up PBR environment shader constant values
    pbr_mat pbr_SetupMaterialPBR(pbr_env env, Color albedo, int metalness, int roughness)
    {
        pbr_mat mat;
        ...
        Texture2D bitmaptemplate = { 0 };

        mat.albedo.bitmap = bitmaptemplate;
        mat.normals.bitmap = bitmaptemplate;
        mat.metalness.bitmap = bitmaptemplate;
        mat.roughness.bitmap = bitmaptemplate;
        mat.ao.bitmap = bitmaptemplate;
        mat.emission.bitmap = bitmaptemplate;
        mat.height.bitmap = bitmaptemplate;
        ...
        const static int pbr_shader_values[7] = {3, 4, 5, 6, 7, 8, 9};
        // Set up PBR shader material texture units
        SetShaderValue(mat.env.pbrShader, mat.albedo.bitmapLoc, &pbr_shader_values[1], 1);
        SetShaderValue(mat.env.pbrShader, mat.normals.bitmapLoc, &pbr_shader_values[2], 1);
        SetShaderValue(mat.env.pbrShader, mat.metalness.bitmapLoc, &pbr_shader_values[3], 1);
        SetShaderValue(mat.env.pbrShader, mat.roughness.bitmapLoc, &pbr_shader_values[4], 1);
        SetShaderValue(mat.env.pbrShader, mat.ao.bitmapLoc, &pbr_shader_values[5], 1);
        SetShaderValue(mat.env.pbrShader, mat.emission.bitmapLoc, &pbr_shader_values[6], 1);
        SetShaderValue(mat.env.pbrShader, mat.height.bitmapLoc, &pbr_shader_values[7], 1);

        return mat;
    }

    // Set texture to PBR material
    void pbr_SetMaterialTexturePBR(pbr_mat *mat, pbr_type type, Texture2D texture)
    {
        switch (type)
        {
        case PBR_ALBEDO:
    ...
    void pbr_UnsetMaterialTexturePBR(pbr_mat *mat, pbr_type type)
    {
        switch (type)
        {
        case PBR_ALBEDO:
    ...
    Light pbr_CreateLight(int type, Vector3 pos, Vector3 targ, Color color, pbr_env env)
    {
        Light light = {0};

        if (pbr_lightsCount < MAX_LIGHTS)
        {
            light.enabled = true;
            light.type = type;
            light.position = pos;
            ...
            enabledName[7] = '0' + pbr_lightsCount;
            typeName[7] = '0' + pbr_lightsCount;
            posName[7] = '0' + pbr_lightsCount;
            targetName[7] = '0' + pbr_lightsCount;
            colorName[7] = '0' + pbr_lightsCount;

            light.enabledLoc = GetShaderLocation(env.pbrShader, enabledName);
            light.typeLoc = GetShaderLocation(env.pbrShader, typeName);
            light.positionLoc = GetShaderLocation(env.pbrShader, posName);
            light.targetLoc = GetShaderLocation(env.pbrShader, targetName);
            light.colorLoc = GetShaderLocation(env.pbrShader, colorName);

            pbr_UpdateLightValues(env, light);
            pbr_lightsCount++;
        }
    ...
    pbr_env pbr_LoadEnvironment(const char *filename, int cubemapSize, int irradianceSize, int prefilterSize, int brdfSize)
    {
        pbr_env env = {0};
        ...
        // Set up environment shader texture units
            const static int values[3] = {0, 1, 2};
        SetShaderValue(env.pbrShader, GetShaderLocation(env.pbrShader, "irradianceMap"), &values[0], 1);
        SetShaderValue(env.pbrShader, GetShaderLocation(env.pbrShader, "prefilterMap"), &values[1], 1);
        SetShaderValue(env.pbrShader, GetShaderLocation(env.pbrShader, "brdfLUT"), &values[2], 1);

        // Set up cubemap shader constant values
        SetShaderValue(cubeShader, GetShaderLocation(cubeShader, "equirectangularMap"), &values[0], 1);

        // Set up irradiance shader constant values
        SetShaderValue(irradianceShader, GetShaderLocation(irradianceShader, "environmentMap"), &values[0], 1);

        // Set up prefilter shader constant values
        SetShaderValue(prefilterShader, GetShaderLocation(prefilterShader, "environmentMap"), &values[0], 1);

        // Set up skybox shader constant values
        SetShaderValue(env.skyShader, GetShaderLocation(env.skyShader, "environmentMap"), &values[0], 1);
        ...
        for (unsigned int i = 0; i < 6; i++)
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, cubemapSize, cubemapSize, 0, GL_RGB, GL_FLOAT, NULL);
        ...
        for (unsigned int i = 0; i < 6; i++)
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, irradianceSize, irradianceSize, 0, GL_RGB, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        ...
        for (unsigned int i = 0; i < 6; i++)
            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, prefilterSize, prefilterSize, 0, GL_RGB, GL_FLOAT, NULL);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    void pbr_UpdateLightValues(pbr_env env, Light light)
    {
        ...
        SetShaderValue(env.pbrShader, light.positionLoc, position, 3);
    ...
    void pbr_UpdateEnvironmentValues(pbr_env env, Camera camera, Vector2 res)
    {
    ...
    void pbr_DrawModelPBR(Model model, pbr_mat mat, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale)
    {
    ...
    // void pbr_DrawSkybox(Shader sky, Texture2D cubemap, Camera camera)
    void pbr_DrawSkybox(pbr_env env, Camera camera)
    {
    ...
    void pbr_UnloadMaterialPBR(pbr_mat mat)
    {
        if (mat.albedo.useBitmap)
            UnloadTexture(mat.albedo.bitmap);
        if (mat.normals.useBitmap)
    ...
    void pbr_UnloadEnvironment(pbr_env env)
    {
